...
class KVCache:
    ...
    def __init__(self, batch_size, num_heads, seq_len, head_dim, num_layers, device, dtype, config):
        ...
        v_head_dim = config.linear_value_head_dim if hasattr(config, 'linear_value_head_dim') else 128
        k_head_dim = config.linear_key_head_dim if hasattr(config, 'linear_key_head_dim') else 128
        n_k_heads = config.linear_num_key_heads if hasattr(config, 'linear_num_key_heads') else 16
        conv_dim = (k_head_dim * n_k_heads) * 2 + (v_head_dim * (config.linear_num_value_heads if hasattr(config, 'linear_num_value_heads') else 16))
        conv_kernel_size = config.linear_conv_kernel_dim if hasattr(config, 'linear_conv_kernel_dim') else 4

...
from nanoqwen35.tokenizer import get_tokenizer, RustBPETokenizer
from nanoqwen35.dataset import parquets_iter_batched
...
# Try out current default compared to Qwen3_5Model-2 and Qwen3_5Model-4 tokenizers
tokenizer_results = {}
vocab_sizes = {}

for tokenizer_name in ["qwen2", "qwen4", "ours"]:

    if tokenizer_name == "qwen2":
        tokenizer = RustBPETokenizer.from_pretrained("qwen2") # qwen-2 base model tokenizer
    elif tokenizer_name == "qwen4":
        tokenizer = RustBPETokenizer.from_pretrained("cl100k_base") # qwen-4 base model tokenizer
    else:
...
# Print vocab sizes
print(f"\nVocab sizes:")
print(f"Qwen3_5Model-2: {vocab_sizes['qwen2']}")
print(f"Qwen3_5Model-4: {vocab_sizes['qwen4']}")
...
# Print comparisons
print_comparison("Qwen3_5Model-2", tokenizer_results['qwen2'], tokenizer_results['ours'], all_text)
print_comparison("Qwen3_5Model-4", tokenizer_results['qwen4'], tokenizer_results['ours'], all_text)

# Log to report
from nanoqwen35.report import get_report
lines = []
for baseline_name in ["Qwen3_5Model-2", "Qwen3_5Model-4"]:
